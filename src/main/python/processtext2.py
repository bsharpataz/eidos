...
def find_where_articles_start(fn):
    line_counter = 0
    final_line = 0
    with open(fn) as f:
        for line in f:
            ...
            if line.strip() == "Narrowed by":
                final_line = line_counter + 2
            line_counter += 1
    if final_line is 0:
        print(f"Didn't find final line - {fn}")
    return final_line
...
def mk_file_lexicon(filename):
    ...
    articles_start = find_where_articles_start(filename)
    if articles_start > 0:
        articles_start += 1
    ...
    print(f"file: {filename}, articles start: {articles_start}")
    with open(filename) as f:
        lines = f.readlines()[articles_start:]
        found_end = False
        curr_article = []
        for line in lines:
            # print(line)
            if line.strip() == end_sign:
                articles.append(" ".join(curr_article).strip())
                curr_article = []
            # if not found_end:
            else:
                curr_article.append(line)
...
def main():
    ...
    for file in glob.glob("*.txt"):
        articles, _, _ = mk_file_lexicon(file)
