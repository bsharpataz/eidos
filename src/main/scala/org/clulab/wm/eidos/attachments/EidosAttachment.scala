...
object EidosAttachment {
  ...
  def newEidosAttachment(json: JValue): EidosAttachment = {
    ...
    (json \ TYPE).extract[String] match {
      ...
      case Location.label => new LocationTM(trigger, someQuantifications)
...
@SerialVersionUID(1L)
class LocationTM(trigger: String, quantifiers: Option[Seq[String]], triggerMention: Option[TextBoundMention] = None,
               quantifierMentions: Option[Seq[Mention]] = None) extends TriggeredAttachment(trigger, quantifiers, triggerMention, quantifierMentions) {

  override def canEqual(other: Any): Boolean = other.isInstanceOf[LocationTM]

  override def newJLDAttachment(serializer: JLDEidosSerializer): JLDEidosAttachment =
    newJLDTriggeredAttachment(serializer, LocationTM.kind)

  override def toJson(): JValue = toJson(LocationTM.label)
}
...
object LocationTM {
  ...
  def apply(trigger: String, quantifiers: Option[Seq[String]]) = new LocationTM(trigger, quantifiers)

  def apply(mention: Mention): LocationTM = {
    val attachmentInfo = TriggeredAttachment.getAttachmentInfo(mention, argument)

    new LocationTM(attachmentInfo.triggerText, attachmentInfo.quantifierTexts, attachmentInfo.triggerMention, attachmentInfo.quantifierMentions)
  }
}
